...
dMaxSectons=np.array([39.6037911036, 42.5543824874, 45.0944839762, 42.3058183336, 44.6564335164,\
             ...
             61.4, 63.6035953284, 67.4789914, 39.8691725099, 50.1914778869, 59.640318022,\
...
#        print preums, lastScan
        ...
        davgTH = a[8]
        if preums and AAA:
            mappt = np.load(fname[:-32] + '_matDatafuncTimeTot.npy') #dt ,vol, area, shape, dmax, dmaxTH, thVol, tort, curv
...
#            print "thth", np.mean(th_thickness)
            if scanIndex == 8:
                th_thickness *= np.NaN
...
#            print int(thCoverage),patchFileName, '\n'
#            dmGrowth1cm =

            #plot for each patient the hemodyn variables
#            scanId +=1
#            OSI_total[:,scanId] = OSI[:]
#
#            OSI_total[:,0] = OSI
...
#                    print 'iiiiiiiiiiii'
                ...
                if lastScan:
                        localLumAreaVarMax,localLumAreaVarSD,localLumAreaVarMin,localLumAreaVar, localLumAreaVarMean = np.nan,np.nan,np.nan, np.ones([600,1])*np.nan, np.nan
                        localThThVarMax,localThThVarSD,localThThVarMin,localThThVar, localThThVarMean = np.nan,np.nan,np.nan, np.ones([600,1])*np.nan, np.nan
                        localGradAvgVarMax,localGradAvgVarSD,localGradAvgVarMin,localGradAvgVar, localGradAvgVarMean =np.nan, np.nan,np.nan, np.ones([600,1])*np.nan, np.nan
                        localDivAvgVarMax,localDivAvgVarSD,localDivAvgVarMin,localDivAvgVar, localDivAvgVarMean =np.nan, np.nan,np.nan, np.ones([600,1])*np.nan, np.nan
                        localDistToCtrlVarMax,localDistToCtrlVarSD,localDistToCtrlVarMin,localDistToCtrlVar, localDistToCtrlVarMean = np.nan,np.nan, np.nan,np.ones([600,1])*np.nan, np.nan
                        localOSIVarMax,localOSIVarSD,localOSIVarMin,localOSIVar, localOSIVarMean =np.nan, np.nan,np.nan, np.ones([600,1])*np.nan, np.nan
                        localRRTVarMax,localRRTVarSD,localRRTVarMin,localRRTVar, localRRTVarMean = np.nan,np.nan, np.nan,np.ones([600,1])*np.nan, np.nan
                        localTAWSSVarMax,localTAWSSVarSD,localTAWSSVarMin,localTAWSSVar, localTAWSSVarMean =np.nan, np.nan,np.nan, np.ones([600,1])*np.nan, np.nan
                        localECAPVarMax,localECAPVarSD,localECAPVarMin,localECAPVar, localECAPVarMean =np.nan, np.nan, np.nan,np.ones([600,1])*np.nan, np.nan

                if not AAA:
                        localLumAreaVarMax,localLumAreaVarSD,localLumAreaVarMin,localLumAreaVar, localLumAreaVarMean = 0, 0, 0, np.zeros([600,1]), 0
                        localThThVarMax,localThThVarSD,localThThVarMin,localThThVar, localThThVarMean = 0, 0,0,  np.zeros([600,1]), 0
                        localGradAvgVarMax,localGradAvgVarSD,localGradAvgVarMin,localGradAvgVar, localGradAvgVarMean =0,  0,0,  np.zeros([600,1]), 0
                        localDivAvgVarMax,localDivAvgVarSD,localDivAvgVarMin,localDivAvgVar, localDivAvgVarMean =0,  0,0,  np.zeros([600,1]), 0
                        localDistToCtrlVarMax,localDistToCtrlVarSD,localDistToCtrlVarMin,localDistToCtrlVar, localDistToCtrlVarMean =0,  0,0,  np.zeros([600,1]), 0
                        localOSIVarMax,localOSIVarSD,localOSIVarMin,localOSIVar, localOSIVarMean = 0, 0,0,  np.zeros([600,1]), 0
                        localRRTVarMax,localRRTVarSD,localRRTVarMin,localRRTVar, localRRTVarMean = 0, 0,0,  np.zeros([600,1]), 0
                        localTAWSSVarMax,localTAWSSVarSD,localTAWSSVarMin,localTAWSSVar, localTAWSSVarMean =0,  0, 0, np.zeros([600,1]), 0
                        localECAPVarMax,localECAPVarMax,localECAPVarMin,localECAPVar, localECAPVarMean = 0, 0, 0, np.zeros([600,1]), 0
...
#                print 'nPatientScan ',nPatientScan
                ...
                nPatientScan = 0
#                thctot.append([list(growthAr[:,0]),thc])
...
#        print SEXE
        if SEXE == 'F':
            if dmaxTH>40:
             ...
             if dmaxTH>50:
                dmaxThreshold = True
        dmaxTH_monthlyrisk = dmaxTH_monthly*2>5
        if preums:
            risky = False
...
#        print preums,dmaxThreshold,dmaxTH_monthlyrisk, risky
        ...
        if lastScan:
            dt = np.nan
        l = [lastScan, patientIndex, scanIndex, os.path.basename(fname)[:-23], AAA,AGE,SEXE,IMC,Psys,Pdias,HTA,nRxantiHTA,DLP,STATINES, volLum, thVol,vTot,vTot_monthly,vTot_monthly_1dot5percentthreshold, areaLum, \
             shapeLum, dmaxLum*2, d0Lum*2, davgLum*2, dmaxTH,dmaxThreshold, d0TH*2, davgTH*2, volLum_monthly/1000, \
             areaLum_monthly/100, shapeLum_monthly, dmaxLum_monthly*2, dmaxTH_monthly*2,dmaxTH_monthlyrisk, volTH_monthly/1000, \
             ...
             patchMin[9], patchMin[10], patchMin[11], patchMin[15], patchMin[16],np.percentile(ECAP,5), patchMean[5], patchMean[6], \
             ...
             patchStd[16], np.std(ECAP) ,\
                        ...
                        localECAPVarMax,localECAPVarMin,thCoverage, thCoverageVar,dt, risky]
        if np.inf in l:
...
#    allPatches=np.append(allPatches,scanIdLocal,axis=1)
    ...
    np.savetxt("localData.tab", allPatches,delimiter='\t',header='AbscissaMetric\tAngularMetric\tBoundaryMetric\tClippingArray\tDistanceToCenterlines\tDivergence_average\tGradients_average\tGroupIds\tHarmonicMapping\tOSI\tPatchArea\tRRT\tSector\tSlab\tStretchedMapping\tTAWSS\tth_thickness\tlocalLumAreaVar\tlocalThThVar\tlocalDivAvgVar\tlocalGradAvgVar\tlocalOSIVar\tlocalRRTVar\tlocalTAWSSVar\tlocalECAPVar\tECAP\tlocalDistToCtrl\tlocalDistToCtrlVar\tthrombusGrowthThresh\tstretchGrowthThresh\tdisToCtrlGrowthThresh\tscanId', fmt='%5s')
    np.savetxt(tabfile, pptList, delimiter='\t', header='lastScan\tpatientId\tscanId\tptName\tAAA\tAGE\tSEXE\tIMC\tPsys\tPdias\tHTA\tnRxantiHTA\tDLP\tSTATINES\tvolLum\tvoTH\tvTot\tvTot_monthly\tvTot_monthly_2percentthreshold\tareaLum\tshapeLum\tdmaxLum\td0Lum\tdavgLum\tdmaxTH\tdmaxTH_50mmthreshold\td0TH\tdavgTH\tvolLum_monthly\tareaLum_monthly\tshapeLum_monthly\tdmaxLum_monthly\tdmaxTH_monthly\tdmaxTH_monthly5mmthreshold\tvolTH_monthly\ttortuosity\ttortLum_monthly\tcurvature\tcurvLum_monthly\tDivergence_average_max\tGradients_average_max\tOSI_max\tPatchArea_max\tRRT_max\tTAWSS_max\tThrombus_thickness_max\tECAP_max\tDivergence_average_min\tGradients_average_min\tOSI_min\tPatchArea_min\tRRT_min\tTAWSS_min\tThrombus_thickness_min\tECAP_min\tDivergence_average_mean\tGradients_average_mean\tOSI_mean\tPatchArea_mean\tRRT_mean\tTAWSS_mean\tThrombus_thickness_mean\tECAP_mean\tDivergence_average_std\tGradients_average_std\tOSI_std\tPatchArea_std\tRRT_std\tTAWSS_std\tThrombus_thickness_std\tECAP_std\tlocalLumAreaVarMean\tlocalLumAreaVarMax\tlocalLumAreaVarMin\tlocaleEThVarMean\tlocaleEThVarMax\tlocaleEThVarMin\tlocalDivAvgVarMean\tlocalDivAvgVarMax\tlocalDivAvgVarMin\tlocalGradAvgVarMean\tlocalGradAvgVarMax\tlocalGradAvgVarMin\tlocalOSIVarMean\tlocalOSIVarMax\tlocalOSIVarMin\tlocalRRTVarMean\tlocalRRTVarMax\tlocalRRTVarMin\tlocalTAWSSVarMean\tlocalTAWSSVarMax\tlocalTAWSSVarMin\tlocalECAPVarMean\tlocalECAPVarMax\tlocalECAPVarMin\tthrombusCoverage\tthrombusCoverageVar\tdt\tCummulativeRisk', fmt='%5s')
    np.save("localdata.npy", allPatches)
    np.save("globaldata.npy", pptList)
    pdcolumnlist = ['lastScan','patientId','scanId','ptName','AAA','AGE','SEXE','IMC','Psys','Pdias','HTA','nRxantiHTA','DLP','STATINES','volLum','voTH','vTot','vTot_monthly','vTot_monthly_2percentthreshold','areaLum','shapeLum','dmaxLum','d0Lum','davgLum','dmaxTH','dmaxTH_50mmthreshold','d0TH','davgTH','volLum_monthly','areaLum_monthly','shapeLum_monthly','dmaxLum_monthly','dmaxTH_monthly','dmaxTH_monthly5mmthreshold','volTH_monthly','tortuosity','tortLum_monthly','curvature','curvLum_monthly','Divergence_average_max','Gradients_average_max','OSI_max','PatchArea_max','RRT_max','TAWSS_max','Thrombus_thickness_max','ECAP_max','Divergence_average_mean','Gradients_average_mean','OSI_mean','PatchArea_mean','RRT_mean','TAWSS_mean','Thrombus_thickness_mean','ECAP_mean','Divergence_average_std','Gradients_average_std','OSI_std','PatchArea_std','RRT_std','TAWSS_std','Thrombus_thickness_std','ECAP_std','localLumAreaVarMean','localLumAreaVarMax','localeEThVarMean','localeEThVarMax','localDivAvgVarMean','localDivAvgVarMax','localGradAvgVarMean','localGradAvgVarMax','localOSIVarMean','localOSIVarMax','localRRTVarMean','localRRTVarMax','localTAWSSVarMean','localTAWSSVarMax','localECAPVarMean','localECAPVarMax','thrombusCoverage', 'thrombusCoverageVar']
